...
public interface IGitService : IDisposable
{
    ...
    void Reset(string sha, string mode);
    void Revert(string sha);
    void CherryPick(string sha);
    void CreateTag(string tagName, string sha);
    void CheckoutCommit(string sha);
}
